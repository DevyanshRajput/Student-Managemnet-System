...
public class DialogAssignSubject extends JDialog implements ActionListener {

	private final JPanel contentPanel = new JPanel();
	InfoFaculty f=null;
	static DialogAssignSubject dialog;
	...
	public static void main(String[] args) {
		try {
			dialog = new DialogAssignSubject(null);
	 ...
	 */
	private DialogAssignSubject(InfoFaculty f) {
	...
	public DialogAssignSubject(InfoFaculty f, MainAdmin am) 
	{
	...
	{
		courcenamecombo.setSelectedItem(new DataCourse().getcourcename(f.getCourceCode()));
		 semoryearcombo.setModel(new DefaultComboBoxModel<String>(new DataCourse().getSemorYear(courcenamecombo.getSelectedItem()+"")));
			String[] totalsub=new DataSubject().getSubjectinCource(f.getCourceCode(),f.getSemorYear());
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==assignsubjectbutton)
		{
			...
			{
				InfoFaculty fnew=new InfoFaculty();
			
				fnew.setCourceCode(new DataCourse().getCourcecode(courcenamecombo.getSelectedItem()+""));
				...
				int result=new DataFaculty().assignSubject(f,fnew);
				if(result>0)
				{
					if(am!=null)
					{
						...
						else if(am.viewfacultypanel!=null&&am.viewfacultypanel.isVisible())
						{
							...
							am.viewfacultypanel=new PanelViewFaculty(new DataFaculty().getFacultyInfobyId(f.getFacultyId()),am,am.viewfacultypanel.lastpanel);
