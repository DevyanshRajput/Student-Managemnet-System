...
public class PanelSubject extends JPanel implements ActionListener
{
	...
	public PanelSubject(MainAdmin am)
	{
		this();
	}
	public PanelSubject(MainStudent sm)
	{
		this();
	...
	public PanelSubject(MainFaculty fm)
	{
		this();
		courcenamecombo.setVisible(false);
		semoryearcombo.setVisible(false);
	...
	public PanelSubject(MainFaculty fm,JComponent lastpanel)
	{
		this(fm);
	...
	public PanelSubject(MainStudent sm,JComponent lastpanel)
	{
		this(sm);
		headerlabel.setLayout(null);
		backbutton = new JButton("Back");
...
//		timer.start();
		...
		Courcecode=new DataCourse().getCourcecode();
		...
		if(courcenamecombo.getSelectedIndex()==0 ||semoryearcombo.getSelectedIndex()==0)
		{
			...
			if(e.getSource()==courcenamecombo)
			{
			 ...
			 if(e.getSource()==addsubject)
			 {
				 int sem=semoryearcombo.getSelectedIndex();
				 int index=courcenamecombo.getSelectedIndex();
				 DialogAddSubject sd=new DialogAddSubject(Courcecode[index-1],sem,table);
	...
	{
		 ResultSet st=new DataSubject().getSubjectinfo(courcecode,sem);
