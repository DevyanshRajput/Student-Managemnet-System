...
public class DialogAddSubject extends JDialog implements ActionListener
{
	...
	private static DialogAddSubject dialog;
	/**
	 * Launch the application.
	 */
	public static void main(String[] args) {
		try {
			 dialog = new DialogAddSubject("IT",1,null);
	 ...
	 * Create the dialog.
	 */
	public DialogAddSubject(String courceCode,int sem,JTable datatable) {
		...
		subjectcodefield.setText(new DataSubject().createSubjectcode(Courcecode, sem));
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if(new DataSubject().isExist(Courcecode,semoryear,subjectnamefield.getText()))
		{
			lblError.setVisible(true);
			lblError.setBounds(subjectnamefield.getX(), subjectnamefield.getY()+subjectnamefield.getHeight()-5, 331, 30);
		...
		else
		{
			if(e.getSource()==addsubject)
			{
				...
				InfoSubject su=new InfoSubject();
				...
				int result=new DataSubject().addSubject(su);
				if(result==1)
				{
					ResultSet st=new DataSubject().getSubjectinfo(Courcecode,semoryear);
