...
import smsystem.application.ChatServer;
import smsystem.general.ScrollFunc;
...
public class PanelSubChat extends JPanel implements ActionListener,Runnable {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PanelSubChat frame = new PanelSubChat(null);
	...
	public PanelSubChat(ChatUser user) {
	...
	{
			ArrayList<ChatUser> list=new DataChat().getUserMessages(user);
			...
			EventQueue.invokeLater(new Runnable()
			{
				...
				public void run() 
				{
					...
					{
					new DataChat().addReadBy(readbylist,user.getFromUserId());
					}
				}
	...
	public  void sendmessage()
	{
		if(socket!=null)
		{
			try
			{
				...
				user.setSr_no(new DataChat().getNewSr_no());
				new DataChat().saveMessage(this.user);
	...
	public void RightSidePanel(ChatUser u)
	{
		...
		ScrollFunc.scrollToBottom(chatscroll);
		this.repaint();
		this.revalidate();
	}
	...
	public JPanel formatPanel(ChatUser u,boolean from)
	{
		...
		{
			int diff=TimeFunc.getDayDifference(u.getMessageDate(),NewMessage.getCurrentDate());
			if(diff==0)
			{
				addLabel("TODAY");
			}
		...
		if(from)
		{
			...
			if(!user.getToUserId().contains("Group"))
			{
				...
				else
				{
					...
					{
						if(new DataStudent().isActive(user.getToUserId()))
						{
					...
					{
						if(new DataFaculty().isActive(user.getToUserId()))
						{
							timelabel.setIcon(new ImageIcon("./assets/doubletick.png"));
						}
					}
	...
	public void run() 
	{
		...
		try
		{
					...
					while(socket!=null&&!socket.isClosed()&&(u=(ChatUser)reader.readObject())!=null)
					{
						...
						{
							new DataChat().readBy(u.getSr_no(), u.getReadBy()+user.getFromUserId()+"#");
	...
	class MessageStatusActionListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent arg0) 
		{
			for(int i=0; i<messagestatus.size(); i++)
			{
					...
					StringTokenizer read=new StringTokenizer(new DataChat().getReadBy(m.getSrNo()),"#");
					boolean contain=false;
					while(read.hasMoreTokens())
					{
