...
public class DialogViewStudent extends JDialog implements ActionListener
{
	...
	private static DialogViewStudent dialog;
	/**
	 * Launch the application.
	 */
	public static void main(String[] args) {
		try {
			 dialog = new DialogViewStudent(null);
	 ...
	 * Create the dialog.
	 */
	public DialogViewStudent(MainAdmin am) {
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==semoryearcombo)
		{
			if(semoryearcombo.getSelectedIndex()>0)
			{
				String courcecode=new DataCourse().getCourcecode(courcenamecombo.getSelectedItem()+"");
				int sem=semoryearcombo.getSelectedIndex();
				rollnumbercombo.setModel(new DefaultComboBoxModel<String>(new DataStudent().getRollNumber(courcecode, sem)));
			}
		}
