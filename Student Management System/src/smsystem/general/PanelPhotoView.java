...
public class PanelPhotoView extends JPanel {
	int xpos[];
	JPanel panel[][];
	...
	public PanelPhotoView(PanelFaculty facultyPanel,int maxphoto) {
					  ...
					  @Override
			  			public void mousePressed(MouseEvent e)
			  			{
						  	if(e.getButton()==MouseEvent.BUTTON1)
						  	{
			  					...
			  					InfoFaculty f=new DataFaculty().getFacultyInfobyId(Integer.parseInt(e.getComponent().getName()));
								if(fp.am!=null)
								{
									fp.am.viewfacultypanel = new PanelViewFaculty(f, fp.am, fp);
								...
								{
									fp.fm.viewfacultypanel = new PanelViewFaculty(f, fp.fm, fp);
								...
								{
									fp.sm.viewfacultypanel = new PanelViewFaculty(f, fp.sm, fp);
	...
	public PanelPhotoView(PanelStudent sp,int maxphoto)
	{
		this.maxphotosinrow=maxphoto;
		this.sp=sp;
		xpos=new int[maxphotosinrow];
		...
		setBackground(Color.WHITE);
		  ...
		  for(int i=0; i<row; i++)
		  {
			  ...
			  for(int j=0; j<totalcolumn; j++)
			  {
				  ...
				  panel[i][j].addMouseListener(new MouseAdapter()
						  {
					  			...
					  			public void mousePressed(MouseEvent e)
					  			{
					  				if (e.getButton() == MouseEvent.BUTTON1)
					  				{
					  					InfoStudent s=new DataStudent().getStudentDetails(Integer.parseInt(e.getComponent().getName()));
					  					
					  					if(sp.am!=null)
					  					{
						  					sp.am.viewstudentpanel=new PanelViewStudent(s,sp.am,sp);
					  					...
					  					{
					  						sp.fm.viewstudentpanel=new PanelViewStudent(s,sp.fm,sp);
					  					...
					  					{
					  						sp.sm.viewstudentpanel=new PanelViewStudent(s,sp.sm,sp);
				  ...
				  degreelabel[i][j]=new JLabel();
				  degreelabel[i][j].setVerticalAlignment(SwingConstants.TOP);
				  degreelabel[i][j].setText(new DataCourse().getsemoryear(s.getCourceCode())+"-"+s.getSemorYear()+" "+" ("+s.getCourceCode()+")");
				  degreelabel[i][j].setHorizontalAlignment(SwingConstants.CENTER);
				  degreelabel[i][j].setFont(new Font("Tahoma", Font.PLAIN, changeDegreeFont()));
				  degreelabel[i][j].setBounds(0, profilepiclabel[i][j].getHeight()+25,panel[i][j].getWidth(), 22);
				  panel[i][j].add(degreelabel[i][j]);
