...
public class DialogChangepassword extends JDialog {
	...
	public DialogChangepassword(InfoStudent s)
	{
		this();
		changepasswordbutton.addActionListener(e->
		{
			...
			{
				int result=new DataStudent().changePassword(s.getUserId(), newpasswordfield.getText());
				if(result>0)
				{
	...
	@SuppressWarnings("deprecation")
	public DialogChangepassword(InfoFaculty f)
	{
		this();
		changepasswordbutton.addActionListener(e->
		{
			...
			{
				int result=new DataFaculty().changePassword(f.getFacultyId()+"", newpasswordfield.getText());
				if(result>0)
				{
				JOptionPane.showMessageDialog(null, "Password updated","Message",JOptionPane.INFORMATION_MESSAGE);
				this.dispose();
